...
class OverlayWindow(QWidget):
    ...
    def start_mouse_listener(self):
                ...
                if button == mouse.Button.left:
                    ...
                    if 0 <= pos.x() <= self.width() and 0 <= pos.y() <= self.height():
                        if pressed:
                            ...
                            if self.fading_enabled and self.engine.has_changes:
                                ...
                                if self.engine.lines:
                                    ...
                                    if last_line and len(last_line) > 1:
                                        ...
                                        # Удаляем линию из основного хранилища
                                        self.engine.lines.pop()
                                # Или последнюю фигуру
                                elif self.engine.shapes:
                                    last_shape = self.engine.shapes[-1]
                                    if last_shape:
                                        self.timer_service.add_shape(last_shape, 3.0)
                                        # Удаляем фигуру из основного хранилища
                                        self.engine.shapes.pop()
                                self.engine.has_changes = False
        ...
        def on_move(x, y):
            try:
                if not self.engine or not self.engine.is_active or not self.engine.is_drawing:
                    return
                pos = self.mapFromGlobal(QPoint(x, y))
                # Проверяем, что позиция в пределах экрана
                if 0 <= pos.x() <= self.width() and 0 <= pos.y() <= self.height():
    ...
    def _draw_lines(self, painter):
        ...
        try:
            ...
            print(f"Ошибка в _draw_lines: {e}")
        ...
        try:
            for temp_data in self.engine.temp_lines:
                if 'lines' in temp_data:
                    ...
                    color_data = temp_data.get('color')
                    if color_data:
                        if isinstance(color_data, QColor):
                            color = QColor(color_data)
                        else:
                            color = QColor(*color_data[:4]) if len(color_data) >= 4 else QColor(255, 0, 0, 255)
            ...
            print(f"Ошибка в _draw_temp_lines: {e}")
    ...
    def _draw_shapes(self, painter):
        ...
        try:
            ...
            print(f"Ошибка в _draw_shapes: {e}")
    ...
    def _draw_rectangle(self, painter, shape):
        ...
        try:
            ...
            if isinstance(color, QColor):
                pen = QPen(color, shape.get('width', 3), Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
            else:
                pen = QPen(Qt.red, shape.get('width', 3), Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
            painter.setPen(pen)
            rect = shape.get('rect')
            if rect:
                painter.drawRect(rect)
        except Exception as e:
            print(f"Ошибка в _draw_rectangle: {e}")
    ...
    def _draw_circle(self, painter, shape):
        ...
        try:
            ...
            print(f"Ошибка в _draw_circle: {e}")
    ...
    def _draw_triangle(self, painter, shape):
        ...
        try:
            ...
            print(f"Ошибка в _draw_triangle: {e}")
    ...
    def _draw_straight_line(self, painter, shape):
        ...
        try:
            ...
            print(f"Ошибка в _draw_straight_line: {e}")
    ...
    def _draw_current(self, painter):
        ...
        try:
            ...
            print(f"Ошибка в _draw_current: {e}")
